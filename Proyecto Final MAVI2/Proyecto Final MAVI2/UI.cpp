...
bool UI::IsNextLevelButtonClicked(int x, int y)
{
	return mPlayNextLevelText->getGlobalBounds().contains(x, y);
}
...
void UI::SetNewMenu()
{
	...
	cout << "[NEW MENU]" << endl; // Debug
	mPlayNextLevelText = new Text;
	mPlayNextLevelText->setFont(*mFont);
	mPlayNextLevelText->setFillColor(Color::White);
	mPlayNextLevelText->setOutlineColor(Color::Black);
	mPlayNextLevelText->setOutlineThickness(5.f);
	mPlayNextLevelText->setCharacterSize(40);
	mPlayNextLevelText->setPosition(50.f, 180.f);
	mPlayNextLevelText->setString("PLAY GAME ->");
}
...
void UI::SetImages(RenderWindow& mWindow)
{
	...
	if (!mBackLv1Tx->loadFromFile("Assets/Backgrounds/backgroundmain.png")) { cout << "Error al cargar la textura del fondo" << endl; }
	...
	mBackLv1Sp->setScale(260.f / mBackLv1Tx->getSize().x, 200.f / mBackLv1Tx->getSize().y);
	mBackLv1Sp->setPosition({ -110.f, 5.f });
}
...
void UI::DrawNewMenu(RenderWindow& mWindow)
{

	mWindow.setMouseCursorVisible(true);
	mWindow.draw(*mPlayNextLevelText);
}
